...
Boston_model6 <- select("crim", Boston, reg_method = "lm",interaction = T, nb_pts = 3)

@
First example, using an R dataset "mtcars", with the dependent variable being "mpg".
<<r>>=
library('GA')
library(datasets)
...
mtcars6 <- select("mpg", mtcars, reg_method = "glm", interaction = T, nb_pts = 2, mu = 0.4)
