...
#' @param parent_selection (character) The mechanism to select parents. Selection mechanisms are "prop","prop_random" or "tournament".
...
select <- function(y, dataset, reg_method = NULL, n_iter = 200, pop_size = 2 * n, objective = "AIC",
  ...
  for (i in 1:n_iter){
    iter <- iter + 1
    popsize <- pop_sizes[i]
    ind <- update_generations(y, dataset, ind, objective,
                               pop_size,
                               generation_gap,
